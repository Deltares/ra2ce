...
gdf = gpd.read_file(Path(r"c:\Users\hauth\OneDrive - Stichting Deltares\projects\RA2CE Uncertainty\2024\paper\work_dir_augus_2025\output\damages\Manual_damage_c6_segmented.gpkg"))
# replace nan by 0
gdf = gdf.fillna(0)
...
gdf_result = gdf[['geometry', 'infra_type', 'length']]
gdf_result['AAL'] = data

# plot a map from the gdf to display AAL, using matplotlib
gdf_result.plot(column='AAL', cmap='OrRd', legend=True)



import matplotlib.pyplot as plt
# put the legend colorbar on top of the plot and in horizontal orientation
cbar = plt.gcf().axes[-1]
cbar.set_position([0.3, 0.9, 0.4, 0.03])


plt.title("AAL per link")
plt.show()


# result_csv = Path(r"c:\Users\hauth\OneDrive - Stichting Deltares\projects\RA2CE Uncertainty\2024\paper\work_dir_augus_2025\output\damages\AAL_per_link.gpkg")
#
