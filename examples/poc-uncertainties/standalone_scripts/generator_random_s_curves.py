...
_path_folder = Path("/vulnerability_curves")
...
for i in range(40):
    ...
    final_height = np.random.normal(
        100, 10
    )  # final height for which we reach 100% damage
    ...
    df.to_csv(_path_folder / f"vulnerability_curve_{i}.csv", index=False)
