...
from ra2ce.osm_network_wrapper.ExtremitiesData import ExtremitiesData
...
class OsmNetworkWrapper:
    ...
    def download_graph_from_osm(self) -> MultiDiGraph:
        ...
        polygon_file = self.output_path.parent.joinpath("network", self.network_dict["polygon"])
    ...
    def drop_duplicates(complex_graph: MultiDiGraph) -> MultiDiGraph:
        unique_elements = set()  # This gets updated during the drop_duplicates_in_nodes and drop_duplicates_in_edges

        unique_graph = OsmNetworkWrapper.drop_duplicates_in_nodes(unique_elements=unique_elements, graph=complex_graph)
    ...
    @staticmethod
    def drop_duplicates_in_edges(unique_elements: set, unique_graph: MultiDiGraph, graph: MultiDiGraph):
        """
        ...
        """
        if not unique_elements or not any(unique_elements) or \
                not all(isinstance(item, tuple) for item in unique_elements):
            raise ValueError("""unique_elements cannot be None, empty, or have non-tuple elements. 
        ...
        def validity_check(extremities_tuple) -> bool:
            ...
            return not (extremities.from_to_id is None or extremities.from_to_coor is None or
                        extremities.from_id == extremities.to_id)

        def valid_extremity_data(u, v, data) -> tuple[ExtremitiesData, dict]:
            _extremities_data = ExtremitiesData.get_extremities_data_for_sub_graph(from_node_id=u, to_node_id=v,
        ...
        for _extremity_data, _edge_data in filter(validity_check
            ...
            _coor_combination = (_extremity_data.from_to_coor, _extremity_data.to_from_coor)
            if all(_combination not in unique_elements for _combination in [_id_combination, _coor_combination]):
                edge_attributes = {
                    key: value for key, value in _edge_data.items()
                }
                unique_graph.add_edge(_extremity_data.from_id, _extremity_data.to_id, **edge_attributes)
