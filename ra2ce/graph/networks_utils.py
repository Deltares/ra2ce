...
class HazardUtils:
    ...
    @staticmethod
    def add_hazard_data_to_road_network(gdf, hazard_path, geometry, tolerance=0.00005):
        """
        ...
        road_mapping_dict = HazardUtils.lookup_road_mapping()
        ...
        gdf['length'] = gdf.geometry.apply(line_length)
        # or lines['length'] = lines['geometry'].apply(lambda x: line_length(x))
        ...
        hzds_data = HazardUtils.create_hzd_df(geometry, hazard_path, hzd_names)
        ...
        for iter_, hzd_name in enumerate(hzd_names):
            ...
            if len(hzd_region) == 0:
                gdf['length_{}'.format(hzd_name)] = 0
                gdf['val_{}'.format(hzd_name)] = 0
                continue

            hzd_reg_sindex = hzd_region.sindex
            from tqdm import tqdm
            tqdm.pandas(desc=hzd_name)
            inb = gdf.progress_apply(lambda x: HazardUtils.intersect_hazard(x, hzd_reg_sindex, hzd_region), axis=1).copy()
            ...
            gdf[['length_{}'.format(hzd_name), 'val_{}'.format(hzd_name)]] = inb[['length_{}'.format(hzd_name),
                                                                                       'val_{}'.format(hzd_name)]]
            # save Hazard Map
            # output_path = load_config()['paths']['test_output']
            # filename = 'exposure_from_dump.shp'
            # gdf.to_file(os.path.join(output_path, filename))
