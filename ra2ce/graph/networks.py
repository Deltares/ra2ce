...
class Network:
    def __init__(self, network_config):
        self.network_config = network_config['network']
        self.config = network_config
        self.save_shp = network_config['network']['save_shp']
        self.primary_files = network_config['network']['primary_file']
        self.diversion_files = network_config['network']['diversion_file']
        self.file_id = network_config['network']['file_id']
    ...
    def network_shp(self):
        ...
        edges, lines_merged = merge_lines_shpfiles(lines, self.network_config['shp_unique_ID'], aadt_names, crs)
        logging.info("Function [merge_lines_shpfiles]: executed with properties {}".format(list(edges.columns)))

        edges, id_name = gdf_check_create_unique_ids(edges, self.network_config['shp_unique_ID'])

        if 'data_manipulation' in self.network_config:
            if self.network_config['data_manipulation'] == 'snapping':
                edges = snap_endpoints_lines(edges, self.network_config['snapping_threshold'], id_name, tolerance=1e-7)
                logging.info("Function [snap_endpoints_lines]: executed with threshold = {}".format(self.network_config['snapping_threshold']))
        ...
        if not lines_merged.empty:
            ...
            lines_merged.to_file(os.path.join(self.config["output"], "{}_lines_that_merged.shp".format(self.network_config['analysis_name'])))
            logging.info(
                "Function [edges_to_shp]: saved at {}".format(
                    os.path.join(self.config["output"], "{}_lines_that_merged".format(self.network_config['analysis_name']))))
        ...
        resulting_network.to_pickle(
            os.path.join(self.config["output"], '{}_gdf.pkl'.format(self.network_config['analysis_name'])))
        ...
        logging.info(
            "Function [graph_from_gdf]: executing, with '{}_resulting_network.shp'".format(self.network_config['analysis_name']))

        # Save graph to gpickle to use later for analysis
        nx.write_gpickle(G, os.path.join(self.config["output"], '{}_graph.gpickle'.format(self.network_config['analysis_name'])), protocol=4)
        print("Saved graph to pickle in {}".format(
            os.path.join(self.config["output"], '{}_graph.gpickle'.format(self.network_config['analysis_name']))))

        # Save graph to shapefile for visual inspection
        graph_to_shp(G, os.path.join(self.config["output"], '{}_edges.shp'.format(self.network_config['analysis_name'])),
                     os.path.join(self.config["output"], '{}_nodes.shp'.format(self.network_config['analysis_name'])))
