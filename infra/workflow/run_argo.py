...
class Argo:
    ...
    def submit_single_job(model):
        with Workflow(model.name.replace('_', '-'), generate_name=True, workflow_template_ref="race-workflow") as w:
            Task("ra2ce-argo", image="containers.deltares.nl/ra2ce/ra2ce:latest", command=["/bin/bash", "-c", "--"], args= ["python", "/scripts/run_race.py"])
