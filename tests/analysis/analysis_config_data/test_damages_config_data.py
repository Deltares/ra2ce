...
class TestDamagesConfigData:
    ...
    def test_given_risk_calculation_triangle_and_invalid_year_when_validate_integrity_then_fails(self, risk_calculation_year: int | None):
        # 1. Define test data.
        ...
        assert not _report.is_valid(), "Expected invalid report due to negative risk_calculation_year"
        assert len(_report._errors) == 1
        assert _report._errors[0] == _expected_error

    def test_given_risk_calculation_triangle_and_valid_year_when_validate_integrity_then_succeeds(self):
        # 1. Define test data.
        _data_name = "Invalid Damages Analysis"
        _damages_config = DamagesConfigData(
            name= _data_name,
    ...
    def test_given_any_non_triangle_risk_calculation_when_validate_integrity_then_succeeds(self, risk_calculation_mode: RiskCalculationModeEnum, risk_calculation_year: int | None):
        ...
        # 3. Verify expectations for invalid data.
        assert isinstance(_report, ValidationReport)
        assert _report.is_valid(), "Expected valid report due to non-triangle risk calculation mode"