...
from ra2ce.analysis.analysis_config_data import (
    AdaptationConfigTypes,
    DamagesConfigTypes,
...
class TestAnalysisConfigDataValidator:
    ...
    def test_validate_given_shp_network_without_id_when_multi_link_losses_then_fails(
        ...
        # 1. Define test data.
        _expected_error = "Not possible to create analysis 'Test Analysis' - Shapefile used as source, but no 'file_id' configured for the 'NetworkSection'."
        ...
        # 3. Verify final expectations
        assert not _report.is_valid()
        assert any( _error == _expected_error for _error in _report._errors)
    ...
    def test_validate_given_shp_network_with_id_when_multi_link_losses_then_succeeds(
        ...
        # 2. Run test
        _validator = AnalysisConfigDataValidator(_test_config_data)
        _report = _validator._validate_analysis_network_compatibility()
    ...
    @pytest.mark.parametrize(
        "analysis_type",
        [
            pytest.param(_option, id=_option.__name__)
            for _option in [
                ale
                for ale in LossesConfigDataTypes
                if ale != MultiLinkLossesConfigData
            ]
            + [ade for ade in AdaptationConfigTypes]
            + [ae for ae in DamagesConfigTypes]
        ],
    )
    def test_validate_given_shp_network_without_id_when_any_valid_analysis_given_then_succeeds(
        self, analysis_type: AnalysisConfigDataProtocol
    ):
        # 1. Define test data
        _test_config_data = AnalysisConfigData(
            root_path=test_results,
            ...
            project=ProjectSection(),
            analyses=[
                analysis_type(name="Test Analysis")
            ],
        )
