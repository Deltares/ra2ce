...
class TestRa2ceHandler:
    ...
    def test_speedup_hazard_overlay(self):
        ...
        files_to_copy = [f for f in legacy_output_path.glob("*") if f.is_file() and 'hazard' not in f.name]
        ...
        # Check results
        legacy_output_gdf = geopandas.read_file(legacy_output_path / 'base_graph_hazard_edges.gpkg')
        update_output_gdf = geopandas.read_file(updated_output_path / 'base_graph_hazard_edges.gpkg')

        assert legacy_output_gdf['EV1_ma'].values == pytest.approx(update_output_gdf['EV1_ma'].values), f"EV1_ma values are not equal before and after update."
        assert legacy_output_gdf['EV1_fr'].values == pytest.approx(update_output_gdf['EV1_fr'].values), f"EV1_fr values are not equal before and after update."
